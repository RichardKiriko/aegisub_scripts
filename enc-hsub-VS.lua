...
function encode_vs(subs,sel)
	...
	if res.vtype==".mov(+alpha)" and res.GPUs=="ffmpeg(mov with alpha)" then  --only use avs for encoding transparent mov (ProRes)
		...
		if mov_enc_res.mov_encoder=="qtrle" then
			bat_code=quo(ffmpegpath).." -i "..quo(root_path1.."\\aeg_encode_tmp\\sub_alpha.avs").." -an -c:v qtrle -r "..framerate.." -vsync cfr "..encname..".mov"
		else
			bat_code=quo(ffmpegpath).." -i "..quo(root_path1.."\\aeg_encode_tmp\\sub_alpha.avs").." -an -c:v png -r "..framerate.." -vsync cfr "..encname..".mov"
		end
	...
	if res.audio and res.delAV and (res.GPUs~="ffmpeg(mov with alpha)" and res.vtype~=".mov(+alpha)") then bat_code=bat_code.."\ndel "..quo(target..encname..res.vtype)
